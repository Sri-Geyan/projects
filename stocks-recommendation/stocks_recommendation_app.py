"""
Investment Recommendation System — Stocks / ETFs / Mutual Funds (Global)
Data Source: Yahoo Finance (yfinance)
Features:
- User enters tickers (AAPL, MSFT, SPY, etc.)
- Computes risk/return metrics
- Ranks assets using weighted multi-metric score
...
st.title("📈 Investment Recommendation System — Stocks / ETFs / Mutual Funds")

with st.sidebar:
    st.header("Configuration")
    tickers_input = st.text_area("Enter tickers (comma separated)", 
                                 value="AAPL, MSFT, AMZN, GOOGL, TSLA, NVDA, JPM, V, PG, UNH, SPY, IVV, VOO, QQQ, VTI",
                                 height=120)
...
# Validate user input
if not tickers:
    st.error("Please enter at least one ticker symbol (e.g., AAPL, MSFT, SPY).")
    st.stop()
...
st.subheader("📊 Stock/ETF Ranking (Best → Worst)")
...
# Bar chart of top 10
st.subheader("🏆 Top 10 Ranked Stocks/ETFs")
...
st.markdown("---")
st.caption("This demo supports Global Stocks, ETFs, and Mutual Funds (via Yahoo Finance). This is educational — not financial advice.")
